...
//// Choose two Cities \\\\
private _usefuls = btc_city_all select {!(isNull _x) && !((_x getVariable ["type", ""]) in ["NameLocal", "Hill", "NameMarine","Ambush","LAmbush"]) && !(_x getVariable ["occupied", false])};
...
_usefuls = _cities select {!((_x getVariable ["type", ""]) in ["NameLocal", "Hill", "NameMarine","Ambush","LAmbush"]) && (_x getVariable ["occupied", false])};
